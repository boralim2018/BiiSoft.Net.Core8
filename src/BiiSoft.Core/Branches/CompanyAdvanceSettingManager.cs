...
namespace BiiSoft.Branches
{
    public class CompanyAdvanceSettingManager : BiiSoftValidateServiceBase<CompanyAdvanceSetting, long>, ICompanyAdvanceSettingManager
    {
        ...
        {
            return CompanyAdvanceSetting.Create(input.TenantId, input.CreatorUserId, input.MultiBranchesEnable, input.MultiCurrencyEnable, input.LineDiscountEnable, input.TotalDiscountEnable, input.CustomTransactionNoEnable, input.ClassEnable, input.ContactAddressLevel);
        }

        protected override void UpdateInstance(CompanyAdvanceSetting input, CompanyAdvanceSetting entity)
        {
            entity.Update(input.LastModifierUserId, input.MultiBranchesEnable, input.MultiCurrencyEnable, input.LineDiscountEnable, input.TotalDiscountEnable, input.CustomTransactionNoEnable, input.ClassEnable, input.ContactAddressLevel);
        }
